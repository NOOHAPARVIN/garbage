...
<tbody>
    ...
    $uid = $_SESSION["uuid"];
    $sql =
        "SELECT tbllodgedcomplain.ComplainNumber,tbllodgedcomplain.AssignTo,tbllodgedcomplain.ID as compid,tbllodgedcomplain.Status,tbllodgedcomplain.WasteType,tbluser.ID as uid,tbluser.FullName,tbluser.MobileNumber,tbluser.Email,tbllodgedcomplain.Address from tbllodgedcomplain join tbluser on tbluser.ID=tbllodgedcomplain.UserID where tbllodgedcomplain.UserID=:uid";
    ...
    if ($query->rowCount() > 0) {
        foreach ($results as $row) { ?>
            <tr>
                ...
                <td><?php echo htmlentities($row->WasteType); ?></td>
                <?php if ($row->Status == "") { ?>
                    <td><?php echo "Not Updated Yet"; ?></td>
                <?php } else { ?>
                    <td><?php echo htmlentities(
                        $row->Status
                    ); ?> (Assign to <?php echo htmlentities(
     $row->AssignTo
 ); ?>)</td>
                <?php } ?>
                <td><a href="view-lodged-complain-detail.php?editid=<?php echo htmlentities(
                    $row->compid
